...
<body>
    ...
    <div id="wrap">
        <div id="content" class="view">
			...
			<form action="/admin/request_trader" method="get">
            ...
            <table border="0" cellspacing="1" cellpadding="0" class="admin_table">
                ...
                <col width="100" /><col width="80" /><col width="*" /><col width="40" /><col width="90" /><col width="80" /><col width="50" />
                <thead>
                <tr>
                    <td>선택</td>
                    <td>번호</td>
            ...
            <fieldset class="admin">
                <span class="fieldset_txt">선택한 회원의 등급을</span> <strong>트레이더</strong>
                &nbsp;로 &nbsp;&nbsp;
            ...
            <?php if($total > 0){ ?>
            <div class="paging">
				<a href="/admin/request_trader?page=1"<?php if($page == 1) echo ' class="first_no"' ?>>first</a>
                <?php if($page_start > $page_count){ ?><a href="/admin/request_trader?page=<?php echo $page_start-1 ?>">prev</a><?php } ?><!-- class="prev_no" -->
				<?php for($i = $page_start;$i<=$page_start + $page_count - 1;$i++){ ?>
				<?php if($i > ceil($total / $count)) break; ?>
                <a href="/admin/request_trader?page=<?php echo $i ?>"<?php if($page == $i) echo ' class="this"' ?>><?php echo $i ?></a>
				<?php } ?>
				<?php if($page_start + $page_count <= $total_page){ ?><a href="/admin/request_trader?page=<?php echo $page_start + $page_count ?>" class="next">next</a><?php } ?>
                <a href="/admin/request_trader?page=<?php echo $total_page ?>" class="last<?php if($total_page == $page) echo '_no' ?>">last</a>
            </div>
			<?php } ?>
        </div>        
    </div>
