...
<body>
    ...
    <div id="wrap">
        <div id="content" class="view">
            <h3 style="font-size:1.875em;font-weight:bold;font-style: normal;">미디어룸 등록</h3>

            <? if ($post['midx']) { ?>
			<form action="/admin/media/<?=$post['midx']?>/edit" method="post" id="media_form" enctype="multipart/form-data">
            <? } else { ?>
			<form action="/admin/media/write" method="post" id="media_form" enctype="multipart/form-data">
            <? } ?>
            <div id="strategy_view1" class="strategy_view">
                    ...
                    <!-- <tr>
                        ...
                        <td>
                            ...
                            if (isset($imgs) && is_array($imgs)) {
                                foreach ($imgs as $v) {
                            ?>
                                <img src="/media/<?=$v['save_name']?>"> <?=$v['file_name']?> (<label><input type="checkbox" name="del_files[]" value="<?=$v['fid']?>" />삭제</label>)<br />
                            <?
                                }
                            }
                            ?>
                    ...
                    <tr class='att_file'>
                        ...
                        <td>
                            ...
                            if (isset($atts) && is_array($atts)) {
                                foreach ($atts as $v) {
                            ?>
                                <a href="/media/<?=$v['save_name']?>" target="_blank"><?=$v['file_name']?></a> (<label><input type="checkbox" name="del_files[]" value="<?=$v['fid']?>" />삭제</label>)<br />
                            <?
                                }
                            }
                            ?>
                ...
                <p class="btn_area">
                    ...
                    <a href="/admin/media" title="취소" class="cancel"><span class="ir">취소</span></a>
                </p>
            </div>
			</form>
