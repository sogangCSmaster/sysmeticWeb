...
.
                            ...
                            $(function() {
                                ...
                                $(".reply_area #commentFrm").submit(function(){
                                    ...
                                    $.post("/lounge/<?=$pb['uid']?>/contents/<?=$cidx?>/reply/write", $(this).serialize(), function(data){
                                        if (data.result) {
                                            $('.reply_area').load("/lounge/<?=$pb['uid']?>/contents/<?=$cidx?>/reply", function() {
                                                page = 1;
                                                getComment(page);
                                            });
                                        } else {
                            ...
                            function modifyFrm(f) {
                                if (f.contents.value == "") {
                                    ...
                                    if (confirm('수정하시겠습니까?')) {
                                        $.post("/lounge/<?=$pb['uid']?>/contents/<?=$cidx?>/reply/modify", $(f).serialize(), function(data){
                                            if (data.result) {
                                                $('.reply_area').load("/lounge/<?=$pb['uid']?>/contents/<?=$cidx?>/reply", function() {
                                                    page = 1;
                                                    getComment(page);
                            ...
                            function deleteFrm(cid) {
                                if (confirm('삭제하시겠습니까?')) {
                                    $.post("/lounge/<?=$pb['uid']?>/contents/<?=$cidx?>/reply/delete", {cid: cid}, function(data){
                                        if (data.result) {
                                            $('.reply_area').load("/lounge/<?=$pb['uid']?>/contents/<?=$cidx?>/reply", function() {
                                                page = 1;
                                                getComment(page);
