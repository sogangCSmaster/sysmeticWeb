...
var checking_start_date = '<?php echo $first_available_date ?>';
var checking_end_date = '<?php echo $last_available_date ?>';
...
Highcharts.setOptions({
	...
	$(function(){
			...
			for(var i = 0;i<$('#attach_strategy_form input[type=checkbox]:checked').length;i++){
				...
				var html = '';
					html += '<tr>';
                        ...
                        html += '<td><span class="plus">'+Math.round($($('#attach_strategy_form input[type=checkbox]:checked')[i]).data('percent')*100)/100+'%</span></td>';
						}else if($($('#attach_strategy_form input[type=checkbox]:checked')[i]).data('percent') < 0){
						html += '<td><span class="minus">'+Math.round($($('#attach_strategy_form input[type=checkbox]:checked')[i]).data('percent')*100)/100+'%</span></td>';
						}else{
						html += '<td><span>'+Math.round($($('#attach_strategy_form input[type=checkbox]:checked')[i]).data('percent')*100)/100+'%</span></td>';
						}
                        html += '<td>';
                            html += '<p class="ratio">';
		...
		$('#edit_portfolio_form').submit(function(){
			if(!$('#name').val()){
				$('#name').focus();
				alert('이름을 입력하세요');
				return false;
			...
			var percents_array = [];
			var inputs = $('#edit_portfolio_form input');
		...
		$.each(data.items, function(key, val){
			html += '<dl>';
				...
				html += '<dt>';
					...
					html += '<br /><img src="'+val.items[i].icon+'" alt="'+escapedHTML(val.items[i].name)+'" /> ';
					}

					html += '<h6>'+escapedHTML(val.name)+'</h6>';
...
<body>
    ...
    <div id="wrap">
        ...
        <div id="content" class="view">
			<form action="/portfolios/<?php echo $portfolio['portfolio_id'] ?>/edit" method="post" id="edit_portfolio_form">
            <div class="portfolio_info">
				<!--
                <h4>
                    ETF 레버리지/인버스(0.1억)
                    <button onclick="" name="" title="수정" class="sbtn"><span class="ir">수정</span></button>  
                </h4>
				-->
                <div id="mod_name">
                    <input id="name" name="name" type="text" title="포트폴리오 명" value="<?php echo $portfolio['name'] ?>" required="required" />
            ...
            <table border="0" cellspacing="0" cellpadding="0" class="portfolio" id="portfolio_strategies">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php if(!empty($strategy['system_tool']['logo'])){ ?><img width="120" src="<?php echo $strategy['system_tool']['logo'] ?>" /><?php }else{ ?><?php echo htmlspecialchars($strategy['system_tool']['name']) ?><?php } ?></td>
						<td><?php echo $strategy['daily_values'][0]['basedate']; ?>~
						<br/><?php  echo $strategy['daily_values'][count($strategy['daily_values'])-1]['basedate'] ?></td>
                        <td>
				<span class="<?php echo getSignClass($strategy['daily_values'][count($strategy['daily_values'])-1]['acc_pl_rate'], 'always') ?>"><?php echo round($strategy['daily_values'][count($strategy['daily_values'])-1]['acc_pl_rate'],2) ?>%</span>
                        <td>
                            <p class="ratio">
            ...
            <fieldset>      
                <dl>
                    <dt>기간 설정:</dt>
                    <dd>
                        <input id="start_date" name="start_date" type="text" title="시작" class="datepicker" value="<?php echo getDateString($portfolio['start_date']) ?>" required="required" />  ~
                        <input id="end_date" name="end_date" type="text" title="종료" class="datepicker" value="<?php echo getDateString($portfolio['end_date']) ?>" required="required" /> 
                    </dd>
                    <dt>금액:</dt>
                    <dd>
                        <input id="amount" name="amount" type="text" title="금액" value="<?php echo htmlspecialchars($portfolio['amount']) ?>" style="width:115px;" required="required" onkeyup="inputNumberFormat(this)" /> 
                    </dd>
                    <dd>
            ...
            <div class="result_data">
                ...
                <tbody>
                    <tr>
                        ...
                        <td><span class="<?php echo getSignClass($portfolio_total_profit_rate, 'false') ?>"><b><?php echo $portfolio_total_profit_rate ?>%</b></span></td>
                        <td class="thead">누적수익금액</td>
                        <td><span class="<?php echo getSignClass($portfolio_total_profit, 'false') ?>"><b><?php echo number_format($portfolio_total_profit) ?></b></span></td>
                    </tr>
                </tbody>
                </table>
