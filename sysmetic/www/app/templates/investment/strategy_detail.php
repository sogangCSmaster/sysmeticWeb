...
<head>
    ...
    $(function(){
        ...
        // 리뷰로드
        $('.review_area').load('/strategies/<?=$strategy['strategy_id']?>/reviews');
...
<body>
    ...
    <div class="wrapper">
        ...
        <div class="container">
            ...
            <section class="area in_snb strategy_ranking">
                <div class="page_header">
                    <a href="<?=$strategies_list?>" class="btn_back">상품목록</a>
                    <a href="#" onclick="window.open('/strategies/174/print','print','width=1050, height=700')" class="btn_default">상품 프린트하기</a>
                </div>
...
<!-- 레이어팝업 : 문의하기 -->
<article class="layer_popup request_strategy">
    <div class="dim" onclick="commonLayerClose('request_strategy')"></div>
    <div class="contents">
        <form id="ask_form" name="ask_form" action="/investment/strategies/<?=$strategy['strategy_id']?>/ask" method="post">
        <div class="layer_header">
            <h2>문의하기</h2>
            <button type="button" title="레이어 팝업 닫기" class="btn_close_layer" onclick="commonLayerClose('request_strategy')"></button>
        </div>
        <div class="cont">
            <p class="name" id="ask_strategy_name"></p>
            <div class="form_box">
                <div class="textarea_box">
                    <textarea id="ask_body" name="ask_body" placeholder="문의내용을 입력해주세요."></textarea>
                </div>
            </div>
			...
			<div class="form_box">
				<table>
				<tr>
					<td><img id="zsfImg" src="/zmSpamFree/zmSpamFree.php?zsfimg" alt="여기를 클릭해 주세요."  onclick="this.src='/zmSpamFree/zmSpamFree.php?re&amp;zsfimg='+new Date().getTime()" /></td>
            ...
            <div class="btn_area half">
                <button type="submit" class="btn_common_red">문의</button>
                <a href="javascript:;" class="btn_common_gray" onclick="commonLayerClose('request_strategy')">취소</a>
            </div>
        </div>
        </form>
    </div>
...
<!-- 레이어팝업 : 문의하기 완료 -->
<article class="layer_popup request_strategy_complete">
    <div class="dim" onclick="commonLayerClose('request_strategy_complete')"></div>
    <div class="contents">
        <div class="layer_header">
            <h2>문의하기</h2>
            <button type="button" title="레이어 팝업 닫기" class="btn_close_layer" onclick="commonLayerClose('request_strategy_complete')"></button>
        </div>
        <div class="cont">
            <div class="summary">
                <p class="complete_msg">문의가 등록되었습니다.</p>
                <p class="info_msg">문의 등록 내용 및 답변은 <br />마이페이지에서 확인 가능합니다.</p>
            </div>
            <div class="btn_area">
                <button type="button" class="btn_common_gray" onclick="commonLayerClose('request_strategy_complete')">닫기</button>
            </div>
        </div>
    </div>
</article>
...
<!-- 레이어팝업 : 투자하기 -->
<article class="layer_popup investment">
    <div class="dim" onclick="commonLayerClose('investment')"></div>
    <div class="contents">
    <form id="invest_form" action="/investment/strategies/<?=$strategy['strategy_id']?>/invest" method="post">
        <div class="layer_header">
            <h2>투자요청</h2>
            <button type="button" title="레이어 팝업 닫기" class="btn_close_layer" onclick="commonLayerClose('investment')"></button>
        </div>
        <div class="cont">
            <p class="name" id="invest_strategy_name"></p>
            <div class="form_box long">
                <div class="group top">
                    <div class="row">
                        <strong class="title">연락처</strong>
                        <div class="input_box">
                            <input type="text" id="mobile" name="mobile" value="<?=$_SESSION['user']['mobile']?>" placeholder="연락처" required="required" />
                        </div>
                    </div>
                    <div class="row">
                        <strong class="title">이메일</strong>
                        <div class="input_box">
                            <input type="text" id="email" name="email" value="<?=$_SESSION['user']['email']?>" placeholder="이메일" required="required" />
                        </div>
                    </div>
                </div>
                <div class="group bottom">
                    <div class="row">
                        <strong class="title">투자가입금액</strong>
                        <div class="right">
                            <div class="input_box">
                                <input type="text" id="s_price" name="s_price" placeholder="투자가입금액입력" required="required" onkeyup="inputNumberFormat(this)" maxlength="12" />
                            </div>
                            <p class="summary">* , 없이 숫자만 입력해 주세요.</p>
                        </div>
                    </div>
                    <div class="row">
                        <strong class="title">투자개시시점</strong>
                        <div class="right">
                            <div class="input_box">
                                <input type="text" id="s_date" name="s_date" placeholder="투자개시시점입력" maxlength="10" required="required" onkeyup="inputOnlyNumberDot(this);" class="datepicker" />
                            </div>
                            <p class="summary">* 예시 : 2016.05.28</p>
                        </div>
                    </div>
                    <div class="row">
                        <strong class="title">최대손실한도율설정</strong>
                        <div class="right">
                            <div class="input_box">
                                <input type="text" id="max_loss_per" name="max_loss_per" placeholder="최대손실한도율설정" maxlength="5" onkeyup="inputOnlyNumberDot(this);" required="required" />
                            </div>
                            <p class="summary">%</p>
                        </div>
                    </div>
                ...
                <p class="caution">
                    투자요청 등록만으로 투자가 이루어 지는 것은 아니며,<br />
                    투자요청을 하시면 확인 후 해당 PB가 연락을 드립니다.
                </p>
                <div class="check_area">
                    <input type="checkbox" id="check01" name="check01">
                    <label for="check01">개인정보 제3자 제공 동의</label>
                    <!-- 동의에 체크되지 않았을 경우 아래 div에 show 클래스 추가 -->
                    <div class="must_agree show">
                        <p>
                            투자시작을 위해서는 반드시 <br />
                            개인정보 제3자 제공에 <br />
                            동의하셔야 합니다
                        </p>
                        <button type="button" class="btn_close" title="닫기"></button>
                    </div>
                </div>
                <p class="agree_cont">
                    - 개인정보를 제공 받는 자 :  해당 상품의 담당 PB<br />
                    - 개인정보 제공 목적 : 해당 상품의 투자 관련 상담 및 투자 실행<br />
                    - 제공하는 개인정보 항목 : 이름 / 연락처 / 이메일
                </p>
            </div>
            <div class="btn_area half">
                <button type="submit" class="btn_common_red">투자요청</button>
                <a href="javascript:;" class="btn_common_gray" onclick="commonLayerClose('investment')">취소</a>
            </div>
        </form>
        </div>
    </div>
...
<!-- 레이어팝업 : 투자하기 완료 -->
<article class="layer_popup investment_complete">
    <div class="dim" onclick="commonLayerClose('investment_complete')"></div>
    <div class="contents">
        <div class="layer_header">
            <h2>투자요청</h2>
            <button type="button" title="레이어 팝업 닫기" class="btn_close_layer" onclick="commonLayerClose('investment_complete')"></button>
        </div>
        <div class="cont">
            <div class="summary">
                <p class="complete_msg">투자요청 되었습니다.</p>
                <p class="info_msg">투자한 내역은 마이페이지에서 확인 <br />가능합니다.</p>
            </div>
            <div class="btn_area">
                <button type="button" class="btn_common_gray" onclick="commonLayerClose('investment_complete')">닫기</button>
            </div>
        </div>
    </div>
</article>
...
<!-- 이미지 보기 레이어 -->
<div class="layer_popup preview_img" >
    <div class="dim" onclick="commonLayerClose('preview_img')"></div>
    <div class="contents" style="left:0px;top:0px;">
        <div class="layer_header">
            <h2>실계좌 정보</h2>
            <button type="button" title="레이어 팝업 닫기" class="btn_close_layer" onclick="commonLayerClose('preview_img')"></button>
        </div>
        <div class="cont">
            <div class="layer_photo">
                <img src="" id="show_img" style="max-width:100%;" />
            </div>
        </div>
    </div>
</div>
